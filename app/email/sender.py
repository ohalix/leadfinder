...
from app.email.smtp import send_one, SMTPError
...
def send_leads_email(
    ...
    email_leads = [l for l in leads if l.get("contact_type") == "email" and l.get("normalized_value")]
    ...
    for lead in email_leads:
        ...
        try:
            ...
            repository.record_email_send(
                ...
                subject=subject_tpl, status="failed",
                error=error_msg, run_label=run_label,
            )
            continue

        try:
            send_one(smtp_cfg, recipient, subject, body)
